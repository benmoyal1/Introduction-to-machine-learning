...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        for t in range(self.max_iter_):
            predictio_y = f.compute_output(X=X, y=y)
            grad = f.compute_jacobian(X=X, y=y, y_pred=y)
            rate = self.learning_rate_.lr_step(t=t)
            previus = np.copy(f.weights)

            f.weights = f.weights - rate * grad
            delta = np.linalg.norm(f.weights - previus)

            if self.out_type_ == "average":
                weights_sum += f.weights

            if self.out_type_ == "best" and f.compute_output(X=X, y=y) < minimum:
                minimum = f.compute_output(X=X, y=y)
                min = f.weights

            self.callback_(solver=self, weights=f.weights, val=predictio_y,
                           grad=grad, t=t, eta=rate, delta=delta)

            if delta < self.tol_:
                return self.return_weight(f.weights, t, min,
                                          weights_sum)

        return self.return_weight(f.weights, self.max_iter_, min,
                                  weights_sum)

    def return_weight(self, weights, n_iter, min, weights_sum):
        if self.out_type_ == 'last':
            return weights
        elif self.out_type_ == "average":
            return weights_sum / n_iter
        else:
            return min
