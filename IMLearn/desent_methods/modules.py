...
class L1(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        return np.linalg.norm(x=self.weights, ord=1)
...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        weights = self.weights @ X.T
        expo = np.log(1 + np.exp(weights))
        return np.asarray(
            -np.mean(y.T @ weights - expo))
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        expo = self.weights @ X.T
        sigmoid = np.exp(expo ) / (1 + np.exp(expo ))
        jacob = (y @ X - X.T @ sigmoid) / X.shape[0]
        return - jacob
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        out = self.fidelity_module_.compute_output(**kwargs)
        regu_out = self.lam_ * self.regularization_module_.compute_output(**kwargs)
        return out + regu_out
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        init_regular = self.regularization_module_.compute_jacobian(**kwargs)

        if self.include_intercept_:
            init_regular = np.concatenate(([0], init_regular))

        jacob = self.fidelity_module_.compute_jacobian(**kwargs) + self.lam_ * init_regular
