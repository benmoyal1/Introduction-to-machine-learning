...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        initial_weights = np.random.normal(0, 1, features_num) / np.sqrt(features_num)
        logistic_reg = LogisticModule(initial_weights)
        if self.penalty_ == "none":
            self.coefs_ = self.solver_.fit(logistic_reg, X, y)
        else:
            if self.penalty_ == "l1":
                l1 = L1(initial_weights)
                regu = RegularizedModule(logistic_reg, l1, self.lam_, initial_weights,
                                         self.include_intercept_)
            else:
                l2 = L2(initial_weights)
                regu = RegularizedModule(logistic_reg, l2, self.lam_,
                                         initial_weights,
                                         self.include_intercept_)
            self.coefs_ = self.solver_.fit(regu, X, y)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        expo_dot = 1 + np.exp(-np.dot(X, self.coefs_))
        return 1 / expo_dot
