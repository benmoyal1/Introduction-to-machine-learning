...
class Perceptron(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for it in range(self.max_iter_):
            # misclassified = False
            misclassified_inds = [ind for ind in range(len(X)) if y[ind] * (X[ind] @ self.coefs_) <= 0]
            for i in misclassified_inds:  # range(len(X)):
    ...
    def _loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        return misclassification_error(y_true=y, y_pred=predict)
