...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for i in range(self.iterations_):
            weak_unfit_model = self.wl_()
            fitted_weak_model = weak_unfit_model.fit(X, y * self.D_)
            predictions = fitted_weak_model.predict(X)
            misclass_inds = y != predictions
            err = np.sum(self.D_[misclass_inds])
            self.weights_[i] = 0.5 * np.log(1.0 / err - 1)
            self.D_ *= np.exp(-predictions * y * self.weights_[i])
            self.D_ /= np.sum(self.D_)
            self.models_.append(fitted_weak_model)
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        for i in range(min(T, self.iterations_)):
            preds += self.weights_[i] * self.models_[i].predict(X)
