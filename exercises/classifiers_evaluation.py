...
def compare_gaussian_classifiers():
    ...
    for f in ["gaussian1.npy", "gaussian2.npy"]:
        ...
        fig = make_subplots(rows=1, cols=2,
                            subplot_titles=(
                                rf"$\text{{Gaussian Naive Bayes (accuracy={round(100 * accuracy(y, y_pred_bayes), 2)}%)}}$",
                                rf"$\text{{LDA (accuracy={round(100 * accuracy(y, y_pred_lda), )}%)}}$"))

        # Add traces for data-points setting symbols and colors
        fig.add_traces([go.Scatter(x=x[:, 0], y=x[:, 1], mode='markers',
                                   marker=dict(color=y_pred_bayes, symbol=class_symbols[y],
                                               colorscale=class_colors(3))),
                        go.Scatter(x=x[:, 0], y=x[:, 1], mode='markers',
                                   marker=dict(color=y_pred_lda, symbol=class_symbols[y], colorscale=class_colors(3)))],
                       rows=[1, 1], cols=[1, 2])

        # Add `X` dots specifying fitted Gaussians' means
        fig.add_traces([go.Scatter(x=bayes_model.mu_[:, 0], y=bayes_model.mu_[:, 1], mode="markers",
        ...
        for i in range(3):
            fig.add_traces([get_ellipse(bayes_model.mu_[i], np.diag(bayes_model.vars_[i])),
